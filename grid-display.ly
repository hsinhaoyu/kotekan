...
#(define (system-with-grid-nums sys)
  (let* ((top-staff (system-top-staffsymbol sys))
         (left-x    0)
         (sp        (staff-space sys))
         (sys-idx (ensure-system-index sys))
         (elements  (ly:grob-object sys 'elements))
         (staff-list (and (ly:grob-array? elements) (ly:grob-array->list elements)))
         (first-staff (and (pair? staff-list) (car staff-list)))
         (staff-extent (and first-staff (ly:grob-extent first-staff sys Y)))
         (staff-top    (if (pair? staff-extent) (cdr staff-extent) 0))
         (top-y        (+ staff-top (* 0.5 sp))))
    (format #t "HERE ~a ~a ~a\n" left-x top-y sp)
    ...
    (if (or (not (number? left-x))
        ...
        (let* ((bn0     (+ FIRST-BAR-NUMBER (* sys-idx MEASURES-PER-SYSTEM)))
